...
async function setupDb() {
  ...
  for (let year = 2020; year <= 2025; year++) {
    const bloomDay = String(14 + (year % 10)).padStart(2, "0");
    await env.DB.exec(`INSERT OR IGNORE INTO sakura_observations (location_id, year, bloom_date, full_bloom_date, source, updated_at) VALUES ('tokyo', ${year}, '${year}-03-${bloomDay}', '${year}-04-01', 'jma', '${year}-04-01')`);
  }
}
...
describe("GET /v1/sakura/historical", () => {
  ...
  it("filters by year range", async () => {
    const res = await app.fetch(
      new Request("http://localhost/v1/sakura/historical?city=tokyo&from_year=2023&to_year=2025", {
        headers: { "X-API-Key": "test-key-123" },
      }),
      env
    );
